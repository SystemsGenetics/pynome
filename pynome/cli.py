...
@click.pass_context
@click.option('--config', default='pynome_config.json',
              type=click.Path(exists=True))
def pynome(ctx, config):
    """
    This is the function which the command line invocation of pynome calls.
    """
    ...
    # The dictionary.get() function is used here, as it returns `None` if the
    ...
    ctx.obj['as'] = AssemblyStorage(
        ...
        irods_base_path=ctx.obj['config']["storage_config"].get(
            "irods_base_path"),
    )
...
def prepare(ctx):
    ...
    for assembly in tqdm(ctx.obj['as'].query_local_assemblies(),
                         desc='Preparing assembly files.'):

        ctx.obj['as'].decompress(assembly)
        ctx.obj['as'].hisat_index(assembly)
        ctx.obj['as'].gtf(assembly)
        ctx.obj['as'].splice_site(assembly)
...
def discover(ctx):
    ...
    if ctx.obj['config']['ensembl_config'].get('metadata_uri'):
    ...
    if ctx.obj['config']['ensembl_config'].get('metadata_uri'):
        # Search for matching taxonomy IDs within the species.txt metadata
        # file, and update the assemblies with that information.
        ...
        for key, update_dict in tax_id_update:
            ctx.obj['as'].update_assembly(key, update_dict)
